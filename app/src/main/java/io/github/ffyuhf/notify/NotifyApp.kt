...
import io.github.ffyuhf.notify.util.AppLogger
...
class NotifyApp : Application() {
    ...
    companion object {
        ...
        /** 固定通知静默渠道ID：重发响铃开关关闭时使用；Android 8.0+ 提醒行为由渠道决定，
     ...
     * 创建通知渠道
     *
     * Android 8.0+ 必须创建通知渠道才能显示通知。创建三个渠道：
     * - CHANNEL_PINNED：用于用户创建的固定通知，高重要性
     * - CHANNEL_PINNED_SILENT：固定通知静默渠道，低重要性
    ...
    private fun createNotificationChannels() {
        ...
        // 固定通知静默渠道 - 低重要性：重发响铃开关关闭时按此渠道发布，
        // 无声音、无震动、无角标、无横幅；createNotificationChannels 幂等，
        // 老用户升级后自动创建
